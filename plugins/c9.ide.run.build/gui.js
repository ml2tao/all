define(function(require, module, exports) {
    ...
    function main(options, imports, register) {
        ...
        function load(){
            ...
            menus.addItemByPath("调试/构建", new ui.item({
                command: "build"
            }), c += 100, plugin);
            menus.addItemByPath("调试/取消构件", new ui.item({
                command: "stopbuild"
            }), c += 100, plugin);
            menus.addItemByPath("调试/构建系统/", mnuBuildSystem, 
                c += 100, plugin);
            menus.addItemByPath("调试/显示构建结果", new ui.item({
                onclick: function(){
                    commands.exec("showoutput", null, { id: "build" });
                }
            }), c += 100, plugin);
            menus.addItemByPath("调试/~", new ui.divider(), c += 100, plugin);
            menus.addItemByPath("调试/自动构建支持的文件", new ui.item({
                type: "check",
                checked: "user/build/@autobuild"
            }), c += 100, plugin);
            menus.addItemByPath("调试/在构建时保存全部", new ui.item({
                type: "check",
                checked: "project/build/@saveall"
            }), c += 100, plugin);
            
            c = 0;
            menus.addItemByPath("调试/构建系统/自动选择", new ui.item({
                type: "radio",
                value: "auto"
            }), c += 100, plugin);
            menus.addItemByPath("调试/构建系统/~", new ui.divider(), c += 100, plugin);
            menus.addItemByPath("调试/构建系统/~", new ui.divider(), c += 1000, plugin);
            menus.addItemByPath("调试/构建系统/新构建系统", new ui.item({
