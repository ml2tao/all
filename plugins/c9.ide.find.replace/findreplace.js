define(function(require, exports, module) {
    ...
    function main(options, imports, register) {
        ...
        function load(){
            ...
            menus.addItemByPath("查找/查找...", new ui.item({
                command: "find"
            }), 100, plugin);
            menus.addItemByPath("查找/查找下一个", new ui.item({
                command: "findnext"
            }), 200, plugin);
            menus.addItemByPath("查找/查找上一个", new ui.item({
                command: "findprevious"
            }), 300, plugin);
            menus.addItemByPath("查找/~", new ui.divider(), 400, plugin);
            menus.addItemByPath("查找/替换...", new ui.item({
                command: "replace"
            }), 500, plugin);
            menus.addItemByPath("查找/替换下一个", new ui.item({
                command: "replacenext",
            }), 600, plugin);
            menus.addItemByPath("查找/替换上一个", new ui.item({
                command: "replaceprevious",
            }), 700, plugin);
            menus.addItemByPath("查找/替换全部", new ui.item({
                command: "replaceall"
            }), 800, plugin);
