define(function(require, exports, module) {
    ...
    function main(options, imports, register) {
        ...
        function load(){
            ...
            menus.addItemByPath("帮助/", mnuHelp, 900, plugin);
            ...
            menus.addItemByPath("Gospel/关于Gospel", new ui.item({ 
                onclick: function(){ showAbout(); }
            }), 100, plugin);

            var c = 0;
            // menus.addItemByPath("帮助/检查", new ui.item({ 
            //     onclick: function(){window.open('http://status.c9.io'); }
            // }), c += 100, plugin);

            menus.addItemByPath("帮助/~", new ui.divider(), c += 100, plugin);

            menus.addItemByPath("帮助/访问社区", new ui.item({ 
                onclick: function(){ 
                    analytics.track("访问Gospel社区");
                    window.open("http://bbs.dodora.cn"); 
                }
            }), c += 100, plugin);
            
            menus.addItemByPath("帮助/~", new ui.divider(), c += 300, plugin);
            
            menus.addItemByPath("帮助/阅读文档", new ui.item({ 
                onclick: function(){ 
                    window.open("http://bbs.dodora.cn"); 
                }
            }), c += 100, plugin);
            menus.addItemByPath("帮助/意见建议", new ui.item({
                onclick: function() {
                    // draw();
                    window.open('http://bbs.dodora.cn');
                }
            }), c += 100, plugin);
            // menus.addItemByPath("帮助/Go To YouTbe", new ui.item({ 
            //     onclick: function(){ 
