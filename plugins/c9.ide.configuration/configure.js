define(function(require, exports, module) {
    ...
    function main(options, imports, register) {
        ...
        function load() {
            ...
            menus.addItemByPath("Gospel/~", new ui.divider(), 300, plugin);
            menus.addItemByPath("Gospel/控制台", new ui.item({
                onclick: function() { window.open(options.dashboardUrl); }
            }), 310, plugin);
            
            menus.addItemByPath("Gospel/~", new ui.divider(), 350, plugin);
            menus.addItemByPath("Gospel/打开项目偏好设置", new ui.item({
                onclick: editProjectSettings
            }), 400, plugin);
            menus.addItemByPath("Gospel/打开用户设置", new ui.item({
                onclick: editUserSettings
            }), 400, plugin);
            menus.addItemByPath("Gospel/打开快捷键设置", new ui.item({
                onclick: function(){
                    kbprefs.editUserKeys();
                }
            }), 600, plugin);
            menus.addItemByPath("Gospel/打开初始化脚本", new ui.item({
                onclick: editInitJs
            }), 700, plugin);
            menus.addItemByPath("Gospel/打开样式表", new ui.item({
                onclick: editStylesCss
            }), 800, plugin);
            
            if (!c9.hosted) {
                menus.addItemByPath("Gospel/重启Gospel", new apf.item({
                    command: "restartc9"
                }), 2000080, plugin);
            } else {
                menus.addItemByPath("Gospel/重新加载工作空间", new apf.item({
                    command: "restartc9vm"
                }), 2000080, plugin);
            }
