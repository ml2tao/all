define(function(require, exports, module) {
    ...
    function main(options, imports, register) {
        ...
        function setMenus() {
            ...
            menus.addItemByPath("编辑/~", new ui.divider(), c += 100, handle);
            menus.addItemByPath("编辑/选择/", null, c += 100, handle);
            menus.addItemByPath("编辑/行/", null, c += 100, handle);
            menus.addItemByPath("编辑/文本/", null, c += 100, handle);
            menus.addItemByPath("编辑/注释/", null, c += 100, handle);
            menus.addItemByPath("编辑/代码伸缩/", null, c += 100, handle);
    
            c = 0;

            addEditorMenu("编辑/行/缩进", "indent"),
            addEditorMenu("编辑/行/取消缩进", "outdent"),
            addEditorMenu("编辑/行/上移行", "movelinesup"),
            addEditorMenu("编辑/行/下移行", "movelinesdown"),

            menus.addItemByPath("编辑/行/~", new ui.divider(), c += 100, handle);
            addEditorMenu("编辑/行/复制当前行并上移一行", "copylinesup"),
            addEditorMenu("编辑/行/复制当前行并下移一行", "copylinesdown"),

            menus.addItemByPath("编辑/行/~", new ui.divider(), c += 100, handle);
            addEditorMenu("编辑/行/删除行", "removeline"),
            addEditorMenu("编辑/行/从当前位置删除行到末尾", "removetolineend"),
            addEditorMenu("编辑/行/从当前位置删除行到首位", "removetolinestart"),

            menus.addItemByPath("编辑/行/~", new ui.divider(), c += 100, handle);
            addEditorMenu("编辑/行/从当前位置分割行", "splitline");
    
            c = 0;

            addEditorMenu("编辑/注释/关闭 | 打开注释", "togglecomment");
    
            c = 0;

            addEditorMenu("编辑/文本/从当前位置删除右边一个字符", "removewordright"),
            addEditorMenu("编辑/文本/从当前位置删除左边一个字符", "removewordleft"),

            menus.addItemByPath("编辑/文本/~", new ui.divider(), c += 100, handle);
            addEditorMenu("编辑/文本/摆放", "alignCursors");
            addEditorMenu("编辑/文本/重置字母", "transposeletters");

            menus.addItemByPath("编辑/文本/~", new ui.divider(), c += 100, handle);
            addEditorMenu("编辑/文本/转为大写", "touppercase"),
            addEditorMenu("编辑/文本/转为小写", "tolowercase");
    
            c = 0;

            addEditorMenu("编辑/代码伸缩/展开 | 折叠代码", "toggleFoldWidget"),
            addEditorMenu("编辑/代码伸缩/展开代码", "unfold"),

            menus.addItemByPath("编辑/代码伸缩/~", new ui.divider(), c += 100, handle);
            addEditorMenu("编辑/代码伸缩/折叠其它", "foldOther");
            addEditorMenu("编辑/代码伸缩/折叠所有", "foldall");
            addEditorMenu("编辑/代码伸缩/展开所有", "unfoldall");
    
            c = 0;
            
            addEditorMenu("编辑/选择/选择全部", "selectall"),
            addEditorMenu("编辑/选择/分割成行", "splitIntoLines"),
            addEditorMenu("编辑/选择/单独选择", "singleSelection"),
            
            menus.addItemByPath("编辑/选择/~", new ui.divider(), c += 100, handle);
            menus.addItemByPath("编辑/选择/多重选择/", null, c += 100, handle);
            
            menus.addItemByPath("编辑/选择/~", new ui.divider(), c += 100, handle);
            addEditorMenu("编辑/选择/选择右边文本", "selectwordright"),
            addEditorMenu("编辑/选择/选择左边文本", "selectwordleft"),
            
            menus.addItemByPath("编辑/选择/~", new ui.divider(), c += 100, handle);
            addEditorMenu("编辑/选择/选择到文本末尾", "selecttolineend"),
            addEditorMenu("编辑/选择/选择到文本首位", "selecttolinestart"),
            
            menus.addItemByPath("编辑/选择/~", new ui.divider(), c += 100, handle);
            addEditorMenu("编辑/选择/选择到文档末尾", "selecttoend");
            addEditorMenu("编辑/选择/训责到文档开头", "selecttostart");
            
            c = 0;
            
            addEditorMenu("编辑/选择/多重选择/Add Cursor Up", "addCursorAbove"),
            addEditorMenu("编辑/选择/多重选择/Add Cursor Down", "addCursorBelow"),
            addEditorMenu("编辑/选择/多重选择/Move Active Cursor Up", "addCursorAboveSkipCurrent"),
            addEditorMenu("编辑/选择/多重选择/Move Active Cursor Down", "addCursorBelowSkipCurrent"),
            
            menus.addItemByPath("编辑/选择/多重选择/~", new ui.divider(), c += 100, handle);
            addEditorMenu("编辑/选择/多重选择/Add Next Selection Match", "selectMoreAfter"),
            addEditorMenu("编辑/选择/多重选择/Add Previous Selection Match", "selectMoreBefore"),
            
            menus.addItemByPath("编辑/选择/多重选择/~", new ui.divider(), c += 100, handle);
            addEditorMenu("编辑/选择/多重选择/Merge Selection Range", "splitIntoLines");
            ...
            menus.addItemByPath("文件/~", new ui.divider(), 1400, handle);
            menus.addItemByPath("文件/换行符模式/", new ui.menu({
            ...
            menus.addItemByPath("文件/换行符模式/Windows (CRLF)", new ui.item({
                type: "radio",
                value: "windows",
                group: grpNewline
            }), 200, handle);

            menus.addItemByPath("文件/换行符模式/Unix (LF)", new ui.item({
                type: "radio",
                value: "unix",
                group: grpNewline
            }), 300, handle);
