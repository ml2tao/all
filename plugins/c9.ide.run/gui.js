define(function(require, module, exports) {
    ...
    function main(options, imports, register) {
        ...
        function load(){
            ...
            // Menus
            var c = 1000;
            menus.setRootMenu("调试", 600, plugin);
                ...
                isAvailable: function(){
                    ...
                    if (process && process.running) {
                        ...
                        if (runner) {
                            ...
                            itmRun.setAttribute("command", "run");
                            itmRun.setAttribute("caption", "运行");
                            return false;
                        }
                    }
                }
            });
            menus.addItemByPath("调试/Run", itmRun, c += 100, plugin);
            var itmRunLast = new ui.item({
                command: "runlast",
                isAvailable: function(){
                    if (process && process.running || !lastRun) {
                        itmRunLast.setAttribute("caption", "最后一次运行");
                        return false;
                    }
                    else {
            ...
            menus.addItemByPath("调试/最后一次运行", itmRunLast, c += 100, plugin);
            menus.addItemByPath("调试/~", new ui.divider(), c += 100, plugin);
            ...
            menus.addItemByPath("调试/运行/", mnuRunAs, c += 100, plugin);
            menus.addItemByPath("调试/运行历史/", new ui.item({
                isAvailable: function(){ return false; }
            }), c += 100, plugin);
            menus.addItemByPath("调试/运行配置/", mnuRunCfg, c += 100, plugin);
            
            menus.addItemByPath("调试/~", new ui.divider(), c += 1000, plugin);
            // menus.addItemByPath("调试/Enable Source Maps", new ui.item({
            //     type    : "check",
            //     checked : "project/debug/@sourcemaps"
            // }), c += 100, plugin);
            menus.addItemByPath("调试/程序崩溃时显示Debugger", new ui.item({
                type: "check",
                checked: "user/debug/@autoshow"
            }), c += 100, plugin);

            c = 0;
            menus.addItemByPath("调试/运行配置/~", new ui.divider(), c += 1000, plugin);
            menus.addItemByPath("调试/运行配置/创建运行配置", new ui.item({
                value: "new-run-config"
            }), c += 100, plugin);
            menus.addItemByPath("调试/运行配置/管理...", new ui.item({
                value: "manage"
            }), c += 100, plugin);

            c = 0;
            menus.addItemByPath("调试/运行/~", new ui.divider(), c += 1000, plugin);
            menus.addItemByPath("调试/运行/新的调试器", new ui.item({
                value: "new-run-system"
            }), c += 100, plugin);
            menus.addItemByPath("调试/运行/修改调试器", new ui.item({
            ...
            menus.addItemByPath("调试/当前文件", new ui.item({
                onclick: function(){
