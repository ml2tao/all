define(function(require, exports, module) {
    ...
    function main(options, imports, register) {
        ...
        function initMenus(menus) {
            // Menus
            menus.setRootMenu("Gospel", 50, plugin);
            menus.setRootMenu("文件", 100, plugin);
            menus.setRootMenu("编辑", 200, plugin);
            menus.setRootMenu("查找", 300, plugin);
            menus.setRootMenu("View", 400, plugin);
            menus.setRootMenu("前往", 500, plugin);
            // menus.setRootMenu("运行", 600, plugin);
            menus.setRootMenu("工具", 700, plugin);
            menus.setRootMenu("窗口", 800, plugin);
            
            var amlNode = menus.get("Gospel").item;
            if (amlNode && amlNode.$ext)
                amlNode.$ext.className += " c9btn";
            
            menus.addItemByPath("文件/~", new apf.divider(), 1000000, plugin);
    
            menus.addItemByPath("View/~", new apf.divider(), 9999, plugin);
            
            menus.addItemByPath("窗口/预设", null, 10200, plugin);
            menus.addItemByPath("窗口/预设/完整的IDE", new ui.item({
                onclick: function(){ setBaseLayout("default"); }
            }), 100, plugin);
            menus.addItemByPath("窗口/预设/最小化编辑器", new ui.item({
                onclick: function(){ setBaseLayout("minimal"); }
            }), 200, plugin);
            menus.addItemByPath("窗口/预设/Sublime模式", new ui.item({
