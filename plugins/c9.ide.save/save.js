define(function(require, exports, module) {
    ...
    function main(options, imports, register) {
        ...
        function load(){
            ...
            menus.addItemByPath("文件/~", new ui.divider(), 600, plugin);
    
            menus.addItemByPath("文件/保存", new ui.item({
                command: "save"
            }), 700, plugin);

            menus.addItemByPath("文件/另存为...", new ui.item({
                command: "saveas"
            }), 800, plugin);

            menus.addItemByPath("文件/保存全部", new ui.item({
                command: "saveall"
            }), 900, plugin);
            
            menus.addItemByPath("文件/撤销保存", new ui.item({
                command: "reverttosaved"
            }), 1000, plugin);
            menus.addItemByPath("文件/撤销所有保存", new ui.item({
