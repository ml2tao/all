...
define(function(require, exports, module) {
    ...
    function main(options, imports, register) {
        ...
        function load(){
            ...
            // Menu Items
            menus.addItemByPath("Gospel/~", new ui.divider(), 2000000, plugin);
            menus.addItemByPath("Gospel/Quit Cloud9", new ui.item({
                selector: "closeAllWindowsQuit:",
                command: "exit"
            }), 2000100, plugin);

            menus.addItemByPath("窗口/Developer Tools", new ui.item({
                onclick: function(){
                    win.showDevTools();
                }
            }), 2000000, plugin);
            ...
            win.on("askForQuit", function(quit) {
                ...
                if (message) {
                    question.show("确认退出Gospel?",
                        "你确定要" + (quit ? "退出 Gospel?" : "关闭"),
                        "Gospel将会保存您的所有状态"
                            + "包括未保存的文件，您下次启动Gospel时依然可用",
                        function(){ // yes
