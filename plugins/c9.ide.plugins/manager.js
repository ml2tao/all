    ...
    function main(options, imports, register) {
        ...
        function load() {
            ...
            if (options.devel) {
                ...
                menus.addItemByPath("工具/~", new ui.divider(), 100000, plugin);
                menus.addItemByPath("工具/开发者", null, 100100, plugin);
    
                menus.addItemByPath("工具/开发者/重新加载内置插件...", new ui.item({
                    command: "reloadPlugin"
                }), 1100, plugin);
                
                menus.addItemByPath("工具/开发者/重新加载最后一个插件", new ui.item({
                    command: "reloadLastPlugin",
                    isAvailable: getLastReloaded
                }), 1200, plugin);
            }

            menus.addItemByPath("文件/创建插件", null, 210, plugin);
            Object.keys(TEMPLATES).forEach(function(name){
                menus.addItemByPath("文件/创建插件/" + TEMPLATES[name], new ui.item({
                    onclick: function(){
